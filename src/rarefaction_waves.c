...
#if (EOS == TM)
static double Isentropic_Dens2Temperature_Function(double TempDown,
                                                   void *params);
#endif
...
double FanFunction(double Dens_at_Xi, void *params) {
  ...
  /* Step 1 */
  Velocity_at_Xi_1 = Isentropic_Dens2Velocity(Dens_at_Xi, rarefaction);
  ...
  double Temp_at_Xi =
      Isentropic_Dens2Temperature(Dens_at_Xi, PresUp / DensUp, DensUp);
...
double GetDensInFan(struct Rarefaction *Rarefaction) {
  ...
  Dens_at_Xi =
      RootFinder(FanFunction, (void *)Rarefaction, 0.0, __DBL_EPSILON__,
                 0.5 * (DensMax + DensMin), DensMin, DensMax, __FUNCTION__);
...
void GetHeadTailVelocity(double PresUp, double DensUp, double VelocityUp,
  ...
  if (Right_Yes) {
    *HeadVelocity = Cs_Up * sqrt(1.0 + SQR(VelocityUp)) +
                    VelocityUp * sqrt(1.0 + SQR(Cs_Up));
    *TailVelocity = Cs_Down * sqrt(1.0 + SQR(VelocityDown)) +
                    VelocityDown * sqrt(1.0 + SQR(Cs_Down));
  } else {
    *HeadVelocity = -Cs_Up * sqrt(1.0 + SQR(VelocityUp)) +
                    VelocityUp * sqrt(1.0 + SQR(Cs_Up));
    *TailVelocity = -Cs_Down * sqrt(1.0 + SQR(VelocityDown)) +
...
//========================================= OK
double Isentropic_Dens2Temperature(double DensDown, double TempUp,
                                   double DensUp) {
  double TempDown;
  ...
  TempDown =
      RootFinder(Isentropic_Dens2Temperature_Function, (void *)&rafaction, 0.0,
                 __DBL_EPSILON__, 5.0, 1.0, 10.0, __FUNCTION__);
...
double Isentropic_Dens2Temperature_Function(double TempDown, void *params) {
  ...
  Expr = 1.5 * TempDown * TempDown +
         TempDown * sqrt(2.25 * TempDown * TempDown + 1.0);
  Expr /= K;
  Expr = pow(Expr, 1.5);
...
#elif (EOS == TM)
  Dens = 1.5 * Temperature * Temperature +
         Temperature * sqrt(2.25 * Temperature * Temperature + 1.0);
...
double Isentropic_Pres2Temperature(struct Rarefaction *Rarefaction) {
  ...
  Temperature = RootFinder(Isentropic_TemperatureFunction, (void *)Rarefaction,
                           0.0, __DBL_EPSILON__, 0.5, 0.1, 10.0, __FUNCTION__);
...
#elif (EOS == TM)
  Expression = pow(TempDown, 5.0 / 3.0) *
               (1.5 * TempDown + sqrt(2.25 * TempDown * TempDown + 1.0));
...
#elif (EOS == TM)
  Pres = pow(Temperature, 5.0 / 3.0) *
         (1.5 * Temperature + sqrt(2.25 * Temperature * Temperature + 1.0));
...
double Isentropic_Dens2Velocity(double DensDown, struct Rarefaction *upstream) {
  ...
  gsl_odeiv2_driver *d = gsl_odeiv2_driver_alloc_y_new(
      &sys, gsl_odeiv2_step_rk8pd, ini_step, abserr, relerr);
