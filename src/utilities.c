...
int GetAllInfomation(struct InitialCondition *IC, struct RiemannProblem *RP) {
  ...
  PresStar = RootFinder(PresFunction, (void *)IC, 0.0, __DBL_EPSILON__,
                        0.5 * (lb + up), lb, up, __FUNCTION__);
  ...
  switch (Pattern) {
    case 1:
      ...
      GetShockVelocity(PresLeft, DensLeft, VelocityLeft, PresStar,
                       DensDown_Left, &ShockVelocity_Left, NULL);

      DensDown_Right = GetDensDown(PresRight, DensRight, PresStar);

      GetShockVelocity(PresRight, DensRight, VelocityRight, PresStar,
                       DensDown_Right, NULL, &ShockVelocity_Right);

      if (ShockVelocity_Left > 0.0)
        VelocityStar =
            GetVelocityDown(PresRight, DensRight, ShockVelocity_Right, PresStar,
                            DensDown_Right);
      else
        VelocityStar = GetVelocityDown(PresLeft, DensLeft, ShockVelocity_Left,
                                       PresStar, DensDown_Left);
    ...
    case 2:
      ...
      GetHeadTailVelocity(PresLeft, DensLeft, VelocityLeft, PresStar,
                          DensDown_Left, VelocityStar, &HeadVelocity_Left,
                          &TailVelocity_Left, false);
      ...
      VelocityStar = GetVelocityDown(PresRight, DensRight, ShockVelocity_Right,
                                     PresStar, DensDown_Right);
    ...
    case 3:
      ...
      GetHeadTailVelocity(PresRight, DensRight, VelocityRight, PresStar,
                          DensDown_Right, VelocityStar, &HeadVelocity_Right,
                          &TailVelocity_Right, true);
...
double U2V(double U) { return U / sqrt(1.0 + U * U); }  // FUNCTION : U2V
